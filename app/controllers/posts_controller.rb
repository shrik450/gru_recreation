...
class PostsController < ApplicationController
  ...
  def rate
    @next_post = Post.random_top_100_post_unrated_by(T.must(current_user))
    render "rate"
  end

  def rate_index
    unrated_post = Post.random_top_100_post_unrated_by(T.must(current_user))
